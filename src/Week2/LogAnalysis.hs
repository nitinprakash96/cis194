...
{-
 ...
 failed disk, and a bored yet incompetent programmer, the log messages are
 horribly out of order. Until we do some organizing, there
 ...
 Note that MessageTree is a recursive data type: the Node constructor itself
 takes two children as arguments, representing the left and
 right subtrees, as well as a LogMessage. Here, Leaf represents the empty tree.

 A MessageTree should be sorted by timestamp: that is, the timestamp of a LogMessage
 in any Node should be greater than all timestamps of any LogMessage in the left
 subtree, and less than all timestamps of any LogMessage in the right child.
...
-- This has an advantage of performace over doing list concatenation recursively
inOrder' :: MessageTree -> [LogMessage]
inOrder' Leaf = []
inOrder' n = inOrderUtil n []
  where
    inOrderUtil z xs =
      case z of
        Leaf -> xs
        (Node left root right) -> inOrderUtil left (root : (inOrderUtil right xs))
